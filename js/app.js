...
class AmbientMixer {
  ...
  setupEventListeners() {
    // Handle click with event delegation
    ...
    if (saveButton) {
      saveButton.addEventListener("click", () => this.showSavePresetModal());
    }
    ...
    if (cancelSaveButton) {
      cancelSaveButton.addEventListener("click", () => this.ui.hideModal());
    }
  ...
  async toggleSound(soundId) {
    ...
    if (audio.paused) {
      ...
      // If slider is at 0, default value = 50
      if (volume === 0) {
        volume = 50;
  ...
  // Update main button based on individual sounds
  ...
  showSavePresetModal() {
    ...
    const hasActiveSound = Object.values(this.currentSoundState).some(
      (volume) => volume > 0,
    );
  ...
  saveCurrentPreset() {
    ...
    if (this.presetManager.presetNameExists(name)) {
      alert(`Preset with the name ${name} already exists`);
      return;
    }
